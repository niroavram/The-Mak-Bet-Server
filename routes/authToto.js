...
Router.post("/create-toto-group",requireLogin, (req, res) => {
  ...
  Router.get("/my-toto-group", requireLogin, (req, res) => {
    ...
    .populate("myUsers")
    .populate("admins")
    .populate({path: "totoGames", populate:{path:"events",populate:[{path:"gamesEvent",populate:{path: "gameApi"}},{path:"userBets",populate:[{path: "gameEvents"},{path: "created_by"}]}]}})
      .then((totogroup) => {
        res.json({ totogroup });
      })
      .catch((err) => {
  ...
  Router.get("/group", requireLogin, (req, res) => {
    ...
    TotoGroup.find({ _id: group_id })
      .populate("myUsers")
      .populate("admins")
      .populate({path: "totoGames", populate:{path:"events",populate:{path:"gamesEvent",populate:{path: "gameApi"}}}})
      .then((totogroup) => {
        res.json({ totogroup });
      })
      .catch((err) => {
