...
Router.post("/create-toto-group", requireLogin, (req, res) => {
  ...
  TotoGroup.findOne({ code: code }).then((savedUser) => {
    if (savedUser) {
...
Router.put("/join-toto-group", requireLogin, (req, res) => {
  ...
  TotoGroup.findOne({ code: code }).then((totogroup) => {
    console.log(
      totogroup.myUsers.includes(req.user._id),
      totogroup.myUsers,
      req.user._id
    );
...
Router.put("/admin-add-remove", requireLogin, (req, res) => {
  ...
  if(isAdmin){
    TotoGroup.findOneAndUpdate(
      ...
      if (err) {
        ...
        res
          .status(200)
          .json({ totogroup: result, message: "Mazal Tov You are in the group" });
      }
    });
  }else{
    TotoGroup.findOneAndUpdate(
  ...
  TotoGroup.find({ myUsers: req.user })
    ...
    .populate({
      ...
      populate: {
        path: "events",
        populate: [
          { path: "gamesEvent", populate: { path: "gameApi" } },
          {
            path: "userBets",
            populate: [{ path: "gameEvents" }, { path: "created_by" }],
          },
        ],
      },
    })
